...
> >
> > 强制类型转换运算符：例如在`Person`类中，写这么一段函数：`operator double(){ return this->a; }`，便是重载了强制类型转换运算符。
>
...
> 用于类层次结构中的指针和引用的转换，它在**运行时**检查类型的安全性。它要求**转换的类型必须包含至少一个虚函数**。
>
...
> `dynamic_cast`是在运行时检查类型的安全性，所以存在运行时开销。如果代码中出现了较多的`dynamic_cast`，那么代码本身在结构设计上很可能存在问题！
